...
pub fn eval<H: Host, S: Spec>(opcode: u8, interp: &mut Interpreter, host: &mut H) {
    match opcode {
        ...
        opcode::ENERGYPRICE => host_env::energyprice(interp, host),
        ...
        opcode::ENERGYLIMIT => host_env::energylimit(interp, host),
        opcode::SLOAD => host::sload::<S>(interp, host),
        opcode::SSTORE => host::sstore::<S>(interp, host),
        opcode::ENERGY => system::energy(interp, host),
        ...
        opcode::CHAINID => host_env::network_id::<S>(interp, host),
